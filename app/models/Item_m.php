...
class Item_m extends CI_Model {
    ...
    private function _get_datatables_query() {
        ...
        // $this->db->order_by('p_item.name', 'ASC');
        $i = 0;
	...
	{
		$item = $this->db->get('p_item')->num_rows();
		$barcode = "A".($item + 1);
		$params = [
			'barcode' => $barcode,
	...
	public function edit($post)
	{
		...
		if ($this->fungsi->user_login()->id === 1) {
			$params['stock'] = $post['stock'];
		}
		if($post['image'] != null) {
	...
	public function deleteDataitem($item_id)
	{
	...
	function update_stock_in_out($data)
	{
		$qty = $data['qty'];
		$id = $data['item_id'];
		$sql = "UPDATE p_item SET stock = stock - '$qty' WHERE item_id = '$id'";
	...
	function update_stock_out_in($data)
	{
		$qty = $data['qty'];
		$id = $data['item_id'];
		$sql = "UPDATE p_item SET stock = stock + '$qty' WHERE item_id = '$id'";
